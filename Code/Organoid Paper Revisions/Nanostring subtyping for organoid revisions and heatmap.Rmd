---
title: "Nanostring subtyping publication without 67"
...
#Load final distrbutions 
#Setwd
```{r}
setwd("/Users/danielskubleny/Documents/Research/Organoid Nanostring Paper/Revisions_2025/")
...
Nano_SampleInfo <- Nano_SampleInfo %>%
  mutate(exclude = ifelse(grepl("67", tube_id), FALSE, exclude))
...
ggsave("unnormalized_counts_tcga_final.svg", unnormalized_counts_tcga, width=6, height=3)
...
ggsave("normalized_counts_tcga_final.svg", normalized_counts_tcga, width=6, height=3)
...
ggsave("log_diff_tcga_final.svg", log_diff_tcga, width=6, height=3)
...
svg(filename="prefsqn_tcga_nanostring_ffpe_final.svg", width=4.5, height=4)
...
plot(density(t(target)[,1]), 
     ...
     col=colramp[1])
        ...
        for(i in 2:47){lines(density(t(test)[,i]),lwd=2,col=colramp2[i])}}
    text(0, 0.27, labels="RUV-III")
    text(8.81, 0.31, labels="Target")
    title("Pre-FSQN", adj = 0, font.main= 1)
dev.off()

svg(filename="fsqn_tcga_nanostring_ffpe_final.svg", width=4.5, height=4)
par(mar = c(3.1, 4.1, 2.1, 2.1), mgp=c(2, 0.5, 0))
plot(density(t(target)[,1]), 
     main = "",
     xlab = expression("Log"[2]*" Expression"), 
     ...
     col=colramp[1])
        for(i in 2:100){lines(density(t(target)[,i]),lwd=2,col=colramp[i])
        for(i in 2:47){lines(density(t(classify_TCGA)[,i]),lwd=2,col=colramp2[i])}}
...
#Gene expression
TCGA_ffpe_exp = as.data.frame(classify_TCGA)
TCGA_ffpe_exp = tibble::rownames_to_column(TCGA_ffpe_exp,"SampleNames")
TCGA_ffpe_exp = merge(TCGA_ffpe_exp, TCGA_ffpe_names, by="SampleNames")
colnames(TCGA_ffpe_exp)[which(names(TCGA_ffpe_exp) == "Patient.barcodes")] <- "sample_id"
TCGA_ffpe_exp = TCGA_ffpe_exp[,-1]

TCGA_ffpe_exp %>% group_by(sample_id) %>% summarise_all(mean) %>% data.frame() -> TCGA_ffpe_exp
...
plot(density(t(target)[,1]),col=colramp[1],lwd=1,ylim=c(0,.30), xlim=c(-20,20))
for(i in 2:9){lines(density(t(target)[,i]),lwd=1,col=colramp[i])
...
#Gene expression
TME_ffpe_exp = as.data.frame(classify_TME)
TME_ffpe_exp = tibble::rownames_to_column(TME_ffpe_exp,"SampleNames")
TME_ffpe_exp = merge(TME_ffpe_exp, TME_ffpe_names, by="SampleNames")
colnames(TME_ffpe_exp)[which(names(TME_ffpe_exp) == "Patient.barcodes")] <- "sample_id"
TME_ffpe_exp = TME_ffpe_exp[,-1]

TME_ffpe_exp %>% group_by(sample_id) %>% summarise_all(mean) %>% data.frame() -> TME_ffpe_exp
...
#Gene expression
TCGA_org_exp = as.data.frame(classify_TCGA)
TCGA_org_exp = tibble::rownames_to_column(TCGA_org_exp,"SampleNames")
TCGA_org_exp = merge(TCGA_org_exp, TCGA_ffpe_names, by="SampleNames")
colnames(TCGA_org_exp)[which(names(TCGA_org_exp) == "Patient.barcodes")] <- "sample_id"
TCGA_org_exp = TCGA_org_exp[,-1]

TCGA_org_exp %>% group_by(sample_id) %>% summarise_all(mean) %>% data.frame() -> TCGA_org_exp
...
#Gene expression
TME_org_exp = as.data.frame(classify_TME)
TME_org_exp = tibble::rownames_to_column(TME_org_exp,"SampleNames")
TME_org_exp = merge(TME_org_exp, TME_ffpe_names, by="SampleNames")
colnames(TME_org_exp)[which(names(TME_org_exp) == "Patient.barcodes")] <- "sample_id"
TME_org_exp = TME_org_exp[,-1]

TME_org_exp %>% group_by(sample_id) %>% summarise_all(mean) %>% data.frame() -> TME_org_exp
...
subtype_nanostring = gene_specific_nanostring

subtype_nanostring = subtype_nanostring %>% filter(!sample_id =="64C-O #2")
```

#Gene expression 
```{r}
TCGA_ffpe_exp$tissue = "ffpe"
TME_ffpe_exp$tissue = "ffpe"
TCGA_org_exp$tissue = "organoid"
TME_org_exp$tissue = "organoid"

expression_nanostring_TCGA = rbind(TCGA_ffpe_exp,TCGA_org_exp)
expression_nanostring_TME = rbind(TME_ffpe_exp,TME_org_exp)

expression_nanostring_TCGA = expression_nanostring_TCGA %>% filter(!sample_id =="64C-O #2")
expression_nanostring_TME = expression_nanostring_TME %>% filter(!sample_id =="64C-O #2")

```

#Heatmap 
#Heatmap annotation TCGA subtypes, EBV and MSI
```{r}
...
#Public datasets heatmap
#Arrange data - TCGA and ACRG first
FSQN ACRG to TCGA
...
log_edata_fullTCGA <- read.csv("~/Documents/R projects/TCGA Draft 2/log_edata_fullTCGA.csv", row.names=1)
...
#TCGA full FSQN to TCGA target
test = as.matrix(fullTCGA_cancer)
fsqn_cancer_TCGA = quantileNormalizeByFeature(test, target)

public_exp = rbind(fsqn_cancer_ACRG, fsqn_cancer_TCGA)

public_exp <- public_exp[, TCGAfinalgenes]

public_exp = t(public_exp)
```
#Pdata for public data
```{r}
...
```
#Isolate MSI and EBV cases
```{r}
pdata_public = complete_subtype %>% filter(study %in% c("TCGA", "ACRG"))
MSI_check_pcr = MSI_check_pcr %>% dplyr::select(c("patient_id", "MSI_pcr"))

pdata_public = merge(pdata_public,MSI_check_pcr, by = "patient_id", all=TRUE)

#Select the columns of interest and generate missing annotation columns 
pdata_public = pdata_public %>% dplyr::select("patient_id", "TCGA_subtype", "TME_subtype", "MSI_pcr", "ebv_ish")

pdata_public$tissue = "Public"
```

#Heatmap for public 
```{r}

patient_order <- pdata_public$patient_id

pdata_public = tibble::column_to_rownames(pdata_public, "patient_id")

#Order the samples for annotation purposes
matched <- match(patient_order, colnames(public_exp))
public_exp <- public_exp[ , matched]

table(colnames(public_exp)==pdata_public$patient_id) #Check
```

#HEATMAP
```{r}
nanostring_validation <- read.csv("~/Documents/Research/Organoid Nanostring Paper/Revisions_2025/ebv_msi_validation.csv")

#Set up data

exp_nanostring = tibble::column_to_rownames(expression_nanostring_TCGA, "sample_id")
exp_nanostring = exp_nanostring %>% dplyr::select(-tissue)

exp_nanostring = t(exp_nanostring) #Transpose


#Merge for phenodata
nanostring_validation = merge(nanostring_validation, subtype_nanostring, by="sample_id", all=TRUE)

nanostring_validation = nanostring_validation[match(colnames(exp_nanostring), nanostring_validation$sample_id), ]


colnames(nanostring_validation)[which(names(nanostring_validation) == "msi_h")] <- "MSI_pcr"
nanostring_validation = nanostring_validation %>% dplyr::select(c("sample_id","TCGA_subtype","TME_subtype","MSI_pcr","ebv_ish","tissue"))

#Make samplie_id rownames
rownames(nanostring_validation) = NULL
nanostring_validation = tibble::column_to_rownames(nanostring_validation, "sample_id")

# Reorder metadata to match columns 
nanostring_validation = nanostring_validation[colnames(exp_nanostring), , drop = FALSE]

#Adjust strings
nanostring_validation$TCGA_subtype <- str_replace_all(nanostring_validation$TCGA_subtype, c("STAD_GS" = "GS", "STAD_CIN" = "CIN", "STAD_EBV" = "EBV", "STAD_MSI" = "MSI"))
nanostring_validation$TME_subtype <- str_replace_all(nanostring_validation$TME_subtype, c("High_cal" = "High", "Low_cal" = "Low"))
nanostring_validation$MSI_pcr <- str_replace_all(nanostring_validation$MSI, c("neg" = "MSS", "pos" = "MSI-H"))
nanostring_validation$ebv_ish <- str_replace_all(nanostring_validation$ebv_ish, c("neg" = "Negative", "pos" = "Positive"))
nanostring_validation$tissue <- str_replace_all(nanostring_validation$tissue, c("ffpe" = "FFPE", "organoid" = "Organoid"))

pdata_public$TCGA_subtype <- str_replace_all(pdata_public$TCGA_subtype, c("STAD_GS" = "GS", "STAD_CIN" = "CIN", "STAD_EBV" = "EBV", "STAD_MSI" = "MSI"))
pdata_public$TME_subtype <- str_replace_all(pdata_public$TME_subtype, c("High_cal" = "High", "Low_cal" = "Low"))
pdata_public$MSI_pcr <- str_replace_all(pdata_public$MSI, c("MSI-L/MSS" = "MSS"))
pdata_public$ebv_ish <- str_replace_all(pdata_public$ebv_ish, c("neg" = "Negative", "pos" = "Positive"))

# Ensure the annotation columns are factors
nanostring_validation$ebv_ish  <- factor(nanostring_validation$ebv_ish)
nanostring_validation$MSI_pcr  <- factor(nanostring_validation$MSI)
nanostring_validation$TCGA_subtype <- factor(nanostring_validation$TCGA_subtype)
nanostring_validation$TME_subtype <- factor(nanostring_validation$TME_subtype)
nanostring_validation$tissue <- factor(nanostring_validation$tissue)
```

#Combo annotation and gene exp 
```{r}
combo = cbind(public_exp, exp_nanostring)
combo_annot = rbind(pdata_public, nanostring_validation)

colnames(combo_annot)[which(names(combo_annot) == "TCGA_subtype")] <- "TCGA Subtype"
colnames(combo_annot)[which(names(combo_annot) == "tissue")] <- "Tissue"
colnames(combo_annot)[which(names(combo_annot) == "ebv_ish")] <- "EBV ISH"
colnames(combo_annot)[which(names(combo_annot) == "TME_subtype")] <- "TME Subtype"
colnames(combo_annot)[which(names(combo_annot) == "MSI_pcr")] <- "MSI"

ann_colors <- list(
  "EBV ISH" = c("Positive" = "#91D1C2FF", "Negative" = "#D1E5F0"),
  MSI = c("MSI-H" = "#E64B35FF", "MSS" = "#D1E5F0"),
  "TCGA Subtype" = c("CIN" = "#F39B7FFF", "EBV" = "#91D1C2FF", "GS" = "#4DBBD5FF", "MSI" = "#E64B35FF"),
  "TME Subtype" = c("High" = "#925E9FFF", "Low" = "#EFC000FF"),
  Tissue = c("FFPE" = "#D00000", "Organoid" = "#FFBA08" , "Public"= "#3F88C5") 
)
```

#Row scaling public and cluster
```{r}
#Row-scaling: z-score across columns 
combo_scaled <- t(scale(t(combo)))   # rows = genes, scale each gene across public samples
combo_scaled[is.na(combo_scaled)] <- 0    # handle constant rows

#Distance & clustering 
row_dist <- dist(combo_scaled, method = "euclidean")   
row_hc   <- hclust(row_dist, method = "complete")        

#row order as gene names 
row_order <- rownames(combo)[row_hc$order]

#Reorder matrices to match the row_order for plotting
combo_scaled <- combo_scaled[row_order, , drop = FALSE]

#Reorder metadata to match columns 
combo_annot = combo_annot[colnames(combo_scaled), , drop = FALSE]
```

#Heatmap combo
```{r}
#UNIFIED color scale for both plots
min_val <- min(combo_scaled)
max_val <- max(combo_scaled)
#Breaks
my_breaks <- seq(min_val, max_val, length.out = 100)
# Viridis pallete
my_colors <- viridis(99)

heatmap_plot_norow = pheatmap::pheatmap(combo_scaled,
                                  annotation_col = combo_annot,
...
svg(filename="heatmap_nanostring_public_norow.svg", width=10, height=6.5)
heatmap_plot_norow
dev.off()

heatmap_plot = pheatmap::pheatmap(combo_scaled,
                                  annotation_col = combo_annot,
                                  annotation_colors = ann_colors, 
                                  color = my_colors,
                                  breaks = my_breaks,
...
svg(filename="heatmap_nanostring_public.svg", width=10, height=8)
heatmap_plot
dev.off()
```
...
cols_of_interest <- rownames(nanostring_validation)

# Get the full column order from the clustered heatmap object
full_col_order <- heatmap_plot$tree_col$labels[heatmap_plot$tree_col$order]
full_row_order <- heatmap_plot$tree_row$labels[heatmap_plot$tree_row$order]

# Find which of our columns of interest appear in what order
zoomed_cols_ordered <- full_col_order[full_col_order %in% cols_of_interest]

#Prepare data for the zoomed heatmap
zoom_matrix <- combo_scaled[full_row_order, zoomed_cols_ordered]
zoom_annotation <- combo_annot[zoomed_cols_ordered, , drop = FALSE]

zoomed_heatmap = pheatmap::pheatmap(
  zoom_matrix,
  scale= "row",
  cellwidth = 15,
  cellheight = 3,
  color = my_colors,  
  breaks = my_breaks,
  annotation_col = zoom_annotation,
  annotation_colors = ann_colors,
  border_color = "grey30",
  cluster_cols = FALSE, 
  cluster_rows = FALSE,  
...
svg(filename="zoomed_heatmap.svg", width=13, height=4)
zoomed_heatmap
dev.off()
