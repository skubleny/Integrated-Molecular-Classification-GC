...
svg("forestplot_multivar_global_final.svg", width=12, height=10)
forestplot_final
dev.off()
...
lm(MSI_check$MSI_SCORE_MANTIS~MSI_check$MSI)
summary(lm(MSI_check$MSI_SCORE_MANTIS~MSI_check$MSI))

lm(MSI_check$MSI_SCORE_MANTIS~MSI_check$STAD_MSI)
summary(lm(MSI_check$MSI_SCORE_MANTIS~MSI_check$STAD_MSI))
```
#MSI mantis score 
```{r}
msi_mantis_plot = ggplot(MSI_check, aes(x=STAD_MSI,y=MSI_SCORE_MANTIS)) + 
        geom_point() + 
        geom_smooth(method='lm') + 
        xlab("TCGA MSI Score") +
        ylab("MSI Mantis Score") +
        ggtitle("Model MSI Score vs. MSI Mantis Score") +
        ylim(0.2,1.5) + 
        ggplot2::annotate("text", x = 0.2, y = 1.2, size = 6, label = as.expression(bquote(~ R^2 ~ "= 0.859"))) +
        theme_classic() +
                theme(panel.grid.major.y = element_line(colour = "gray", size = 0.15)) +
                theme(panel.grid.minor.y = element_line(colour = "gray", size = 0.1)) +
                theme(panel.grid.major.x = element_line(colour = "gray", size = 0.15)) +
...
ggsave("msi_mantis_plot.svg", msi_mantis_plot, width = 4, height = 4)
```
```{r}
